use crate::{
    Message, SystemState, annotation_list, graphics::GraphicsY, view::DrawingContext,
...
impl WaveData {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                for group_name in all_groups {
                    self.render_group_section(ui, Some(group_name.to_string()), msgs, waves);
                }
    ...
    fn render_group_section(
        ...
        let items: Vec<_> = waves
            .annotations
            .iter()
            .filter(|r| r.group_name() == group_filter)
            .collect();
        ...
        egui::CollapsingHeader::new(format!("{} ({})", display_name, items.len()))
            .id_salt(&display_name)
            .default_open(true)
            .show(ui, |ui| {
                if items.is_empty() {
                    ui.weak("  No items");
                ...
                for (idx, annotation) in items.into_iter().enumerate() {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        */
                        ...
                        // Use the annotation's current name or a default
                        let mut typename = "Arrow";
                        if annotation.get_type() == 1 {
                            typename = "Rectangle";
                        }

                        let current_name = if annotation.get_name().is_empty() {
                            format!("{typename} {}", idx + 1)
                        } else {
                            annotation.get_name()
                        };
                        ...
                        // Group Selector
                        let mut current_group = annotation.group_name();
                        ...
                        egui::ComboBox::from_id_salt(combo_id)
                            ...
                            .show_ui(ui, |ui| {
                                // Option: Ungrouped
                                if ui
                                    .selectable_value(&mut current_group, None, "None (Ungrouped)")
                                    .clicked()
                                {
